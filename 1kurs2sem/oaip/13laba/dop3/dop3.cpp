...
bool findPath(Tree* t, int key, int path[], int& len)
{
    ...
    if (findPath(t->Left, key, path, len) ||
        findPath(t->Right, key, path, len))
        return true;
...
int main()
{
    ...
    for (;;)
    {
        ...
        cout << "4 - вывод дерева\n";
        cout << "5 - число ветвей от корня до ближайшей вершины с заданным ключом.\n";
        ...
        case 5:
        {
            ...
            if (findPath(root, key, path, len)) {
                cout << "Путь: ";
                for (int i = 0; i < len; i++)
                    cout << path[i] << " ";

                cout << "\nКоличество ветвей: " << len - 1 << endl;
            }
            else {
                cout << "Ключ не найден\n";
            }
